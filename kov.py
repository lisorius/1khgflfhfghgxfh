...
available_drink_names = ["Спрайт", "Пепси", "Водка"]
...
@router.message(OrderFood.choosing_drink_name, F.text.in_(available_drink_names))
async def food_chosen(message: Message, state: FSMContext):
    await state.update_data(chosen_food=message.text.lower())
    await message.answer(
        text="Спасибо. Теперь, пожалуйста, выберите размер порции:",
...
@router.message(StateFilter("OrderFood:choosing_drink_name"))
async def food_chosen_incorrectly(message: Message):
    await message.answer(
...
async def food_size_chosen(message: Message, state: FSMContext):
    ...
    await message.answer(
        text=f"Вы выбрали {message.text.lower()} порцию {user_data['chosen_food']}.\n"
             f"Попробуйте теперь заказать напитки или ещё 1 блюдо: /drink",
